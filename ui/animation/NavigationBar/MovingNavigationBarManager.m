...
#import "_greats.h"
#import "MovingNavigationBarManager.h"
...
#pragma mark - MovingNavigationBarManager class

@interface MovingNavigationBarManager () <UIScrollViewDelegate>

/**
 *  @brief Handle statusBar
 */
@property (nonatomic, strong) TLYShyStatusBarController *statusBarController;
/**
 *  @brief Handle navigationBar
 */
@property (nonatomic, strong) TLYShyViewController *navBarController;
/**
 *  @brief Handle extensionView
 */
@property (nonatomic, strong) TLYShyViewController *extensionController;

/**
 *  @brief Handle scrollView or tableView or collectionView
 */
...
@implementation MovingNavigationBarManager
...
- (void)setViewController:(UIViewController *)viewController
{
    ...
    /** @knowledge http://blog.csdn.net/hcy_12345/article/details/48130345
...
- (BOOL)_shouldHandleScrolling
{
    ...
    // 当 页面可见 + ((scrollview 内容高度 多余 视图高度的那部分) 大于 扩展view的总计高度)，则需要处理滚动情况
...
- (void)_handleScrolling
{
    ...
    if (!isnan(self.previousYOffset))
    {
        ...
        // 2 - Ignore any scrollOffset beyond the bounds, 限定滚动处理的位移范围，比如：-64
        ...
        if (fabs(deltaY) > FLT_EPSILON) // @knowledge DBL_EPSILON和 FLT_EPSILON主要用于双精度和单精度的比较当中
        {
            self.contracting = deltaY < 0; // 负向位移 (收缩)
        }
        ...
        // GTH: Calculate the exact point to avoid expansion resistance 伸展阻力
...
#pragma mark - KVO scrollView 's contentSize
                       ...
                       context:(void *)context
{
    ...
    {
        if (self.isViewControllerVisible // 当页面可见
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [self swizzleMethod:@selector(viewWillLayoutSubviews) withMethod:@selector(tly_swizzledViewDidLayoutSubviews)];
        [self swizzleMethod:@selector(viewWillDisappear:) withMethod:@selector(tly_swizzledViewWillDisappear:)];
    });
}
...
- (MovingNavigationBarManager *)shyNavBarManager
{
    id shyNavBarManager = objc_getAssociatedObject(self, &shyNavBarManagerKey);
    if (!shyNavBarManager)
    {
        shyNavBarManager = [[MovingNavigationBarManager alloc] init];
...
- (MovingNavigationBarManager *)_internalShyNavBarManager
{
