...
- (void)viewWillAppear:(BOOL)animated{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        offset.x = _selectedTabIndex * self.contentScrollView.frame.size.width;
        [self.contentScrollView setContentOffset:offset animated:_selectedTabIndex];
        
        CGFloat centerX = (_selectedTabIndex + 0.5 ) * (screen_width / self.btnArray.count);
        if(_selectedTypeAnimate){
            [UIView animateWithDuration:.3 animations:^{
                ...
                [self clickTitleBtn:[self.btnArray objectAtIndex:_selectedTabIndex]];
            }];
        }else{
            [self.indicatorView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(self.titleView.mas_left).offset(centerX);
